...
public class WordDao {
    ...
    public List<Word> searchWord(String str) {          //模糊查询：将输入的字母在数据库中查找
    ...
    public void delete(WordDto wordDto) {                //删除
        List<WordInfo> wordInfos = LitePal.where("key = ?", wordDto.getWordInfo().getKey()).find(WordInfo.class);
    ...
    public boolean add(String word, String pronunciation, String meaning) {          //添加
        Word word1 = new Word(word, pronunciation, meaning);
        return word1.save();
    }

    public boolean modify(String word, String pron, String acce) {                  //修改
